// Package adjustcloud is the cloud-generator tuner: a live unique
// puff behind a panel of the generator knobs. j/k pick a knob, h/l
// change it, s saves the cloud component's config and quits.
package adjustcloud
...
const DefaultConfigPath = "components/cloud/config.json"
...
func (t *Tuner) Nudge(steps int) {
	...
	t.Cfg.Nudge(cloud.Knob(t.Cursor), steps)
}
...
func (m *Model) apply() {
	if err := cloud.Use(m.tuner.Cfg); err != nil {
		m.note = err.Error()
		return
	}
	m.note = ""
...
func (m Model) View() tea.View {
	...
	putText(m.screen, 0, 0, "adjust cloud   j/k select  h/l change  [/] ×10  s save+quit  q quit ", 245)
	for i := 0; i < nKnobs; i++ {
		marker, fg := "  ", 250
		if i == m.tuner.Cursor {
			marker, fg = "> ", 214
		}
		row := marker + fmt.Sprintf("%-11s %s ", cloud.KnobLabel(cloud.Knob(i)), m.tuner.Cfg.Display(cloud.Knob(i)))
