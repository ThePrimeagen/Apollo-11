// Package adjustsky is the blue-sky tuner: the live gradient behind
// a panel of the three knobs — the angle the darker blue comes from,
// the light ink, and the dark ink. j/k pick a knob, h/l change it,
// s saves the sky component's config and quits.
package adjustsky
...
const DefaultConfigPath = "components/sky/config.json"
...
func (t *Tuner) Nudge(steps int) {
	...
	t.Cfg.Nudge(sky.Knob(t.Cursor), steps)
}
	...
	play := screenplay.New(screenplay.Entry{
		...
		Scene: &screenplay.Ensemble{
			Assemble: func() []screenplay.Component {
				return []screenplay.Component{sky.New().At(0.55)}
			},
		},
	})
	play.Start()
...
func (m *Model) apply() {
	if err := sky.Use(m.tuner.Cfg); err != nil {
		m.note = err.Error()
		return
	}
	m.note = ""
...
func (m Model) View() tea.View {
	...
	putText(m.screen, 0, 0, "adjust sky   j/k select  h/l change  [/] ×10  s save+quit  q quit ", 245)
	for i := 0; i < nKnobs; i++ {
		marker, fg := "  ", 250
		if i == m.tuner.Cursor {
			marker, fg = "> ", 214
		}
		row := marker + fmt.Sprintf("%-11s %s ", sky.KnobLabel(sky.Knob(i)), m.tuner.Cfg.Display(sky.Knob(i)))
