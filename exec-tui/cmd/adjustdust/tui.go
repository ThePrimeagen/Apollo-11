...
// Model is the bubbletea shell around the tuner: the mirrored kick
// plus the knob panel. Every change is applied as the active puff.
type Model struct {
	Path    string
	Saved   bool
	w, h    int
...
// Open reads the puff-config JSON at path, makes it the active puff,
// and seeds the tuner from it. A missing or invalid file is an error.
...
// FrameMsg advances the kick one frame.
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case "s":
			...
			if err := m.tuner.Puff.Save(m.Path); err != nil {
...
func (m Model) apply() {
	if err := dust.UsePuff(m.tuner.Puff); err != nil {
		m.note = err.Error()
		return
	}
	m.note = ""
...
func (m Model) panel() {
	putText(m.screen, 0, 0, "adjust dustoff   j/k select  h/l change  [/] ×10  s save+quit  q quit ", 245)
	for i := 0; i < nKnobs; i++ {
		marker, fg := "  ", 250
		if i == m.tuner.Cursor {
			marker, fg = "> ", 214
		}
		row := marker + fmtPad(knobMeta[i].label, 12) + " " + formatKnob(knob(i), m.tuner.get(knob(i))) + " "
