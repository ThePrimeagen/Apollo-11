// checkprio: the Check Priority scene from scenes/checkprio,
// standalone. The house opens on the function's first line over the
// Rose Pine floor, and the scene plays itself: the whole DANZIG
// check as one C-style function — check_for_higher_priority_jobs()
// — reveals one line per beat, then a gold cursor walks it in
// execution order: old = -0, the loop over the eight core sets, the
// read of data[11] (the twelfth word, the PRIORITY word), the
// new-against-old compare, the win, the winner, the run — one
// caption per step — and rests forever on the run line.
//
//	space / p / enter   replay from the top
//	q / ctrl+c          quit
//
//	go run ./cmd/checkprio
//	go run ./cmd/checkprio -seconds 45
package main
...
func newModel(seconds float64) model {
	show := checkprio.New()
	play := screenplay.New(screenplay.Entry{Name: "Check Priority", Scene: show})
	play.Start()
	return model{
		w:       defaultW,
		h:       defaultH,
...
// replay is Stop then Start: a fresh director, back to the reveal.
...
func (m model) status(w int) string {
	dim := "\x1b[38;5;240m"
	reset := "\x1b[0m"
	return dim + pad(" Check Priority   space replay   q quit", w) + reset
}
...
func main() {
	...
	if _, err := termreset.Run(m, opts...); err != nil {
		fmt.Fprintln(os.Stderr, "checkprio:", err)
		os.Exit(1)
	}
}
