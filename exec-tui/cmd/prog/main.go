// prog: the portable program-alarm drop from scenes/prog. The
// north-facing lander falling under twinkling stars, pausing 1202,
// 1202, then 1201. Seven live knobs (four drops, three holds) walk
// 50ms. Play rebuilds from the current knobs; j/k select, h/l walk
// it. q quits.
//
//	p / enter / space   play from the top
//	j / k               select knob
//	h / l               −50ms / +50ms
//	s                   save knobs to scenes/prog/config.json
//	q                   quit
//
//	go run ./cmd/prog
//	go run ./cmd/prog -seconds 15
package main
...
const (
	...
	minH       = 4
	frameMs    = 1000.0 / 30
	statusRows = 1 + int(prog.KnobCount)
)
...
func newModel(seconds float64) model {
	show := prog.New(nil)
	play := screenplay.New(screenplay.Entry{Name: "prog", Scene: show})
	play.Start()
	return model{
		w:       defaultW,
		h:       defaultH,
...
func (m model) move(delta int) model {
	n := int(prog.KnobCount)
	m.cursor = prog.Knob((int(m.cursor) + delta%n + n) % n)
	return m
}
...
func (m model) save() model {
	...
	if err := prog.Use(m.show.Cfg); err != nil {
		m.note = "save failed: " + err.Error()
		return m
	}
	m.note = "saved"
...
func (m model) status(w int) []string {
	...
	hot := "\x1b[38;5;214m"
	reset := "\x1b[0m"
	help := dim + pad("prog   p play  j/k select  h/l ±50ms  s save  q quit", w) + reset
	if m.note != "" {
		help = dim + pad(m.note, w) + reset
	}
	rows := []string{help}
	for i := prog.Knob(0); i < prog.KnobCount; i++ {
		marker, color := "  ", dim
		if i == m.cursor {
			marker, color = "> ", hot
		}
		rows = append(rows, color+pad(fmt.Sprintf("%s%-11s %6.3fs", marker, prog.KnobLabel(i), m.show.Cfg.Value(i)), w)+reset)
	}
	return rows
}
...
func main() {
	...
	cfgPath := flag.String("config", prog.DefaultConfigPath,
		"prog timing JSON; a missing file keeps the stock knobs")
	flag.Parse()
	...
	c, err := prog.LoadOrDefault(cfgFile)
	if err != nil {
		fmt.Fprintln(os.Stderr, "prog:", err)
		os.Exit(1)
	}
	if err := prog.Use(c); err != nil {
		fmt.Fprintln(os.Stderr, "prog:", err)
		os.Exit(1)
	}
	m := newModel(*seconds)
