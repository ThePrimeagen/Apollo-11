...
// The sky tuner edits this component's own config: the JSON lives
// at components/sky/config.json, next to the code it tunes.
...
func TestConfigHome(t *testing.T) {
	...
	t.Run("unhappy: no config hides next to the tuner", func(t *testing.T) {
		if _, err := os.Stat(filepath.Join("..", "..", "cmd", "adjustsky", "config.json")); err == nil {
			t.Fatal("config.json must not live in cmd/adjustsky — it belongs to components/sky")
		}
	})
}
